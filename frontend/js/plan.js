  ...
  renderContent: function() {
    ...
    for (var i = 0; i < filtered.length; i += 7) {
      ...
      var phase = PHASES.filter(function(p) {
        var pts = p.days.split('-');
        return start >= parseInt(pts[0]) && start <= parseInt(pts[1]);
      })[0] || PHASES[0];
      ...
      chunk.forEach(function(d) {
        ...
        var badge = d.isToday ? ' <span style="color:var(--sf);font-size:8px;">← TODAY</span>' : '';
        html += '<div class="dr '+cls+'" onclick="App.openDayLesson('+d.day+')">' +
          '<div class="dnum">'+d.day+'</div>' +
          '<div class="dinfo">' +
            '<div class="dsub">'+d.topic.icon+' '+d.topic.topic+badge+'</div>' +
